...
public final class BindCellAdapter extends BaseObjectRecyclerAdapter<Cell, RVViewHolder> {
    ...
    private CellObserver cellObserver = new CellObserver() {
        public void onCellChange(Cell cell) {
            int position = BindCellAdapter.this.findPositionOfCell(cell);
            if (position != -1) {
                BindCellAdapter.this.notifyItemChanged(position);
            }
        }
    ...
    private View.OnAttachStateChangeListener attachStateChangeListener = new View.OnAttachStateChangeListener() {
        ...
        public void onViewDetachedFromWindow(View v) {
            ...
            for (int i = 0; i < childCount; ++i) {
                ...
                BindCellAdapter.this.notifyViewDetachedFromWindow(viewHolder);
            }

        }
    };

    public BindCellAdapter(Context context) {
        super(context);
    }

    public BindCellAdapter(Context context, List<Cell> dataList) {
    ...
    public RVViewHolder onCreateContentViewHolder(ViewGroup parent, int viewType) {
        ...
        RVViewHolder viewHolder = RVViewHolder.create(viewDataBinding.getRoot());
        ...
        if (layoutManager instanceof GridLayoutManager) {
            ...
            gridLayoutManager.setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {
                public int getSpanSize(int position) {
                    int fixedPosition = BindCellAdapter.this.fixLoopPosition(position);
                    if (BindCellAdapter.this.isContentItem(fixedPosition)) {
                        int validHeaderCount = BindCellAdapter.this.isHeaderEnable() ? BindCellAdapter.this.getHeaderItemCount() : 0;
                        Cell cell = (Cell) BindCellAdapter.this.getDataAt(fixedPosition - validHeaderCount);
